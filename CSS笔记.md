...
1. PS切图按住alt点击小眼睛可单独显示某个图层
2. 通常增加 **padding** 的时候需要考虑是否减去 **width** 和 **height**
3. 当需要（跳转操作）制作按钮时候可以使用a标签
4. 注意高度塌陷，利用设置**背景颜色**进行判断
5. 当出现文字需要排版时，注意**行高** 例如 font: 11px/1，且确定字体大小(对于不同浏览器不同标签默认字体大小可能不一样)
